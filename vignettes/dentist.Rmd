...
```{r file-based-subset}
# Create temporary subset files for the demo
snp_file <- tempfile(fileext = ".txt")
writeLines(region_snps, snp_file)
...
fwrite(sumstat_sub, sub_sumstat, sep = "\t", quote = FALSE)
...
n_samples <- nrow(fread(paste0(sub_bfile, ".fam"), header = FALSE))
...
bim_sub <- fread(paste0(sub_bfile, ".bim"), header = FALSE,
                 col.names = c("chrom", "variant_id", "gd", "pos", "A1", "A2"))
bim_aligned <- bim_sub[match(aligned_sumstat$SNP, bim_sub$variant_id), ]
